// showrooms.js — renders Showrooms from data/showrooms.js (window.KGM_SHOWROOMS).
  ...
  function renderShowrooms(data) {
    ...
    grid.querySelectorAll('[data-directions]').forEach((el) => {
      el.addEventListener('click', () => {
        window.KGM.analytics.track('directions_click', { showroom_name: el.dataset.directions });
      });
    });
    grid.querySelectorAll('.showroom-card').forEach((el) => {
      el.addEventListener('click', () => {
        window.KGM.analytics.track('showroom_view', { showroom_name: el.dataset.showroomId });
      }, { passive: true, once: true });
    });
  }

  window.KGM.showrooms = { renderShowrooms };
})();
